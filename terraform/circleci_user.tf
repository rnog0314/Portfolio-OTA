...
#Create IAM User
resource "aws_iam_user" "ci_adview" {
  name = "ci-${var.name}"
  tags = var.tags
}
...
resource "aws_iam_access_key" "aws_access_key" {
  user = aws_iam_user.ci_adview.name

  depends_on = [
    aws_iam_user.ci_adview
  ]
}
...
resource "circleci_context" "ci_user" {
  depends_on   = [aws_iam_user.ci_adview]
...
# 7d39371f5c3ececada6c76c916dd8299f5b56ab7
